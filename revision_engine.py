...
class RevisionEngine:
    def __init__(self, db_path: str = str(DB_DIR)):
    ...
    def _init_db(self):
        with self._lock:
            ...
            # ⚡ INDEX TỐI ƯU HÓA TRUY VẤN LỌC MASTERY_SCORE
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_user_mastery 
                ON user_vocabulary (user_id, mastery_score)
            """)
    ...
    def review_word(self, user_id: str, word: str, quality: int) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if quality < 3:
                ...
                if rep_count == 0:
                    # Lần học đầu tiên thành công -> Ôn lại sau 6 giờ
                    next_review = now + timedelta(hours=6)
                    interval = 1
                elif rep_count == 1:
                    # Lần thứ 2 -> 24 giờ sau
    ...
    def get_practice_word(self, user_id: str, recent_words: List[str]) -> Optional[Dict[str, Any]]:
        """Sinh tồn vô hạn (Endless Mode) với tỉ lệ trọng số"""
        with self._lock:
            conn = self._get_connection()
            cursor = conn.cursor()
    ...
    # ==========================================================
    # TRUY VẤN LỌC CSDL CHO FASTAPI
    # ==========================================================

    def get_learning_words(self, user_id: str) -> List[Dict[str, Any]]:
        """Lọc từ vựng CHƯA THUỘC (mastery_score < 1.0)"""
        with self._lock:
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute("""
            ...
            return [
                {
                    ...
                    "context_example": r[4],
                    "level": r[5],
                    "mastery_score": float(r[6] or 0.0),
                    "created_at": r[7],
                    "last_reviewed": r[8]
                }
                for r in rows
    ...
    def get_inventory_words(self, user_id: str) -> List[Dict[str, Any]]:
        """Lọc từ vựng ĐÃ THUỘC (mastery_score >= 1.0)"""
        with self._lock:
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute("""
                ...
                FROM user_vocabulary
                WHERE user_id = ? AND mastery_score >= 1.0
                ORDER BY next_review_date DESC, created_at DESC
            """, (user_id,))
            rows = cursor.fetchall()
            return [
                {
