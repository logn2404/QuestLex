...
KEYBERT_MODEL_DIR.parent.mkdir(exist_ok=True, parents=True)
                ...
                for img_name in image_files:
                    ...
                    try:
                        ...
                        for item in top_vocab:
                            word = item['word']
                            mastery = item.get('mastery_score', item.get('mastery', 0.0))

                            print(f"📌 {word.upper()} ({item['pos']})")
                            print(f"   📊 Level: {item.get('level', 'N/A')}")
                            print(f"   📌 Loại từ   : {item.get('pos', 'N/A')}")
                            print(f"   🇻🇳 Nghĩa TV: {item.get('vietnamese_meaning', 'Không có')}")
                            ...
                            print(f"   📈 Mastery: {round(mastery * 100)}%\n")
                        ...
                        print(f"❌ Lỗi khi xử lý file {img_name}: {img_err}\n")
                        continue
