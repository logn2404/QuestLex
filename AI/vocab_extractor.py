...
class VocabularyExtractor:
    ...
    def _fetch_dictionary_info(self, word: str, context: str = "", pos: str = "") -> Dict[str, Any]:
        ...
        if not info["definition"]:
            try:
                ...
                pos_map = {"NOUN": wordnet.NOUN, "VERB": wordnet.VERB, "ADJ": wordnet.ADJ, "ADV": wordnet.ADV} 
    ...
    def _normalize_token_pos(self, token: Any) -> str:
        ...
        if tag.startswith(("NN", "NNS")): return "NOUN" 
    ...
    def extract_candidates(self, text: str, doc=None) -> List[Dict[str, str]]:
        ...
        if self.matcher is not None:
            for _match_id, start, end in self.matcher(doc): 
                span = doc[start:end] 
                if not (2 <= len(span) <= 3): continue 
                if not all(t.is_alpha and t.pos_ != "PROPN" for t in span): continue 
                if any(t.is_stop for t in span): continue 

                phrase = "_".join(t.lemma_.lower() for t in span) 
                if phrase in seen_lemmas or self._is_non_learning_word(phrase): continue 
                if not self._has_dictionary_meaning(phrase, pos="NOUN"): continue 
    ...
    def get_keybert_scores(self, text: str, candidates: List[Dict[str, str]]) -> Dict[str, float]:
        ...
        keywords = self.kw_model.extract_keywords(text, candidates=kb_input, top_n=len(kb_input)) 
        kb_dict = {word.replace(" ", "_"): float(score) for word, score in keywords} 
    ...
    def get_pos_bonus(self, pos: str, lemma: str) -> float:
        pos_bonus = {"NOUN": 0.15, "VERB": 0.05, "ADJ": 0.05, "ADV": 0.02} 
    ...
    def process_text(
        ...
        if level is not None:
            level_thresholds = {"A1": 0.10, "A2": 0.24, "B1": 0.42, "B2": 0.62, "C1": 0.80, "C2": 0.92} 
